...
(function (global) {
  ...
  var COLUMNS = [
    ...
    { key: 'name',      label: 'Name',          required: true, identity: true, from: 'fs',
    ...
    /* Identity, not a correction: written on every row whether or not this is
       the field being put right. Leaving it blank on the rows that did not need
       it made a correction file whose identifying column was empty on 140 of
       150 lines — impossible to check by eye, and broken outright if your
       import is set to match on the serial rather than the name. */
    { key: 'serial',    label: 'Serial Number', identity: true, from: 'onestop', fixes: 'serial-missing',
      help: 'Always written when known, because it identifies the printer.' },
  ...
  function only(row, config, codes, value) {
    ...
    // Identity columns are handled directly and never routed through here;
    // this is belt and braces if one ever is.
    if (codes === 'identity') return value;
  ...
  /* Which rows this file would actually change, and what it would change.
     ...
     Counted by comparing against what Freshservice already holds rather than
     by whether a cell is filled in: the identity columns are written on every
     row and would otherwise be reported as changing all of them. */
  function changeSummary(rows, config) {
    ...
    activeColumns(config).forEach(function (col) {
      ...
      rows.forEach(function (r) {
        ...
        if (!v) return;
        if (N.clean(currentValue(r, col.key)) === N.clean(v)) return;
        n++;
      });
